...
2. SSOT for guardian definitions is `docs/agents/`. A runtime that discovers
   agents elsewhere reaches them by link, path reference, or prompt injection
   — one real copy under `docs/agents/`, projections everywhere else.
...
9. **Fast dispatch payload.** The owner process runs
   ...
   in their definition); escalate the model only when triage cannot return
   the one-line pass. When more than one guardian is owed, the owner
   dispatches them in parallel in one turn. Pre-commit keeps the name-only
   form — no bundle flood at commit.

## REJECTED

- **Guardian rediscovers the batch alone** — each guardian Glob/git for the
  change set and open every ADR frontmatter. Works, but pays full tool and
  model cost on the fast path. Replaced by rule 9 (`--bundle` + cheap tier).
  Would reopen only if a host could not run the dispatch script before
