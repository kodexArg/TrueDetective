...
3. Frontmatter carries exactly these seven fields, in this order:
   ...
   `category` is one of (mapped for this **Project Zomboid profession mod**):
   - `frontend` — player-facing client surface: client Lua, UI strings,
     phrases spoken to the player, character-creation presentation;
   - `backend` — shared game hooks and registration: shared Lua, registries,
     scripts, forage defs, detection logic that is not pure presentation;
   - `devops` — install scripts, Workshop packaging, host paths, CI;
   ...
   owning profession balance: `profession balance` is the topic and is wrong;
   `changing profession cost or XP, editing forage weights, tuning detection
   chance, amending detection Lua under 42.0/media` is the trigger and is right.
...
11. An ADR is the source of truth for the decision it records, and it
    ...
    ADRs — the decisions, then every other document, [[MOD-API]] included. Each
...
- **A `status` field in frontmatter** — `proposed | accepted | deprecated |
  superseded` on every ADR. Dropped because the directory already answers the
  question, and a second answer can disagree with the first. It would reopen
  only if an ADR ever needed to sit in `docs/adrs/` without binding; no such
  need is known.
- **An immutable body with supersession chains** — once accepted, freeze the
  body and spawn a successor for every change of mind. It kept each file
  immutable, but scattered one theme across a chain of numbers. Replaced by
  rule 7 — the theme holds the number, the policy moves through `REJECTED`.
- **Leaving a hollow stub behind on retirement** — the retired file staying
  in place with an empty body. Under rule 6 a file present in `docs/adrs/`
  reads as in force. Closed for as long as rule 6 stands.
- **Mod-only category set without frontend/backend** — considered for a pure
  Lua mod. Rejected so harness-default category enum and tooling stay aligned;
  frontend/backend are remapped in rule 3 to client vs shared game surfaces.
